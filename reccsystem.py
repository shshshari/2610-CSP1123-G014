...
import sqlite3
from reccsystem import QUESTIONS, get_recommendation, generate_reason
...
def quiz():
    if request.method == "POST":
        ...
        stall=stall, reason=reason, mode=mode)

        return render_template_string(QUIZ_PAGE, questions=QUESTIONS)


        # add, delete and update stall for admin only
...
def admin_add_stall():
    ...
    if request.method == "POST":
        ...
        VALUES (?, ?, ?, ?, ?, ?)
        """, (name, category, price_range, vibe, dietary, description))
        conn.commit()
        conn.close()
...
def admin_edit_stall(stall_id):
    ...
    if request.method == "POST":
        ...
        cursor.execute("""
        UPDATE stalls
        SET name=?, category=?, price_range=?, vibe=?, dietary=?, description=?
        WHERE id=?
        """, (name, category, price_range, vibe, dietary, description, stall_id))
        conn.commit()
        conn.close()
        return redirect(url_for("recc.admin_stalls"))
...
def admin_delete_stall(stall_id):
    ...
    <p>💬 {{ reason }}</p>
    {% if mode == "random" %}
    <p>🎲 (Random pick — no strong preference detected)</p>
    {% endif %}
    <br>
    ...
    <a href="/admin/stalls/add">+ Add New Stall</a>
    <br><br>
    <table border="1" cellpadding="8">
    <tr>
    <th>ID</th><th>Name</th><th>Category</th>
    <th>Price</th><th>Vibe</th><th>Dietary</th><th>Actions</th>
    </tr>
    {% for stall in stalls %}
    <tr>
    <td>{{ stall.id }}</td>
